...
import { Button, Input, Card, CardHeader, CardTitle, Badge, Modal } from '../../../shared/ui';
import { useForm, useModal } from '../../../shared/hooks';
...
export const EditProjectPage: React.FC = () => {
    ...
    const { values, handleChange, handleBlur, resetToValues } = useForm<ProjectFormData>({
        name: '',
        description: '',
        startDate: '',
        endDate: '',
            ...
            try {
                ...
                if (projectRes.success && projectRes.data) {
                    ...
                    resetToValues({
                        ...
                        startDate: formatDateForInput(projectRes.data.startDate),
                        endDate: formatDateForInput(projectRes.data.endDate),
                        complexity: projectRes.data.complexity.toString(),
                        status: projectRes.data.status.toString(),
                        budgetEstimate: projectRes.data.budgetEstimate?.toString() || '',
                    });
                } else {
                    showNotification({ type: 'error', message: 'Proyecto no encontrado' });
                ...
                showNotification({ type: 'error', message: 'Error al cargar el proyecto' });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await projectsService.update(id, {
                ...
                status: parseInt(values.status) as ProjectStatus,
                budgetEstimate: values.budgetEstimate ? parseFloat(values.budgetEstimate) : null,
            });

            if (response.success) {
                showNotification({ type: 'success', message: 'Proyecto actualizado exitosamente' });
                navigate(`/projects/${id}`);
            } else {
                showNotification({ type: 'error', message: response.message || 'Error al actualizar proyecto' });
            }
        } catch (error) {
        ...
        <div className="flex flex-col h-full overflow-y-auto">
                ...
                <form className="flex flex-col gap-8" onSubmit={handleSubmit}>
                    ...
                    <Card>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            />
                            <div className="flex flex-col gap-2">
                                <span className="text-slate-700 dark:text-white text-sm font-bold">Estado</span>
                                <select
                                    name="status"
                                    value={values.status}
            ...
            >
                <div className="flex flex-col gap-6">
                    <div className="flex flex-col gap-2">
                        <span className="text-slate-700 dark:text-white text-sm font-bold">Habilidad *</span>
                        <select
                            value={newSkillId}
                    ...
                    <div className="flex flex-col gap-2">
                        <span className="text-slate-700 dark:text-white text-sm font-bold">Nivel Requerido</span>
                        <select
                            value={newSkillLevel}
