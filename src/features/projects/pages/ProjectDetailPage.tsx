...
import { projectsService, skillsService, applicationsService, agentService, usersService, assignmentsService } from '../../../shared/api';
...
export const ProjectDetailPage: React.FC = () => {
    ...
    // Modal para asignar miembro al proyecto
    ...
    // Abrir modal de asignación y cargar usuarios
    const handleOpenAssignModal = async () => {
        ...
        if (orgUsers.length === 0) {
            ...
            try {
                ...
                if (response.success && response.data) {
                    // Filtrar usuarios ya asignados (aprobados)
                    const approvedUserIds = applications
                        .filter(a => a.status === ApplicationStatus.Approved)
                        .map(a => a.userId);
                    const available = response.data.filter(
                        u => u.isActive && !approvedUserIds.includes(u.id)
                    );
                    setOrgUsers(available);
                }
            } catch {
    ...
    const handleAssignUser = async () => {
        ...
        try {
            ...
            if (response.success) {
                showNotification({ type: 'success', message: `${selectedUser?.fullName} asignado exitosamente` });
                assignModal.close();
                // Remover usuario de la lista disponible
                setOrgUsers(prev => prev.filter(u => u.id !== selectedUserId));
                setSelectedUserId('');
            } else {
                showNotification({ type: 'error', message: response.message || 'Error al asignar usuario' });
            }
        } catch {
    ...
    return (
        <div className="flex flex-col h-full overflow-y-auto">
            ...
            {/* Modal Asignar Miembro al Proyecto */}
            <Modal
                ...
                <div className="space-y-5">
                    ...
                    {!loadingUsers && filteredUsers.length === 0 && (
                        <div className="text-center py-8">
                            ...
                            <p className="text-slate-500 dark:text-slate-400">
                                {orgUsers.length === 0 ? 'No hay usuarios disponibles para asignar' : 'No se encontraron resultados'}
                            </p>
                        </div>
                    )}
                    ...
                    <div className="flex justify-end gap-3 pt-2">
                        <Button variant="outline" onClick={assignModal.close}>
                            Cancelar
                        </Button>
                        <Button
