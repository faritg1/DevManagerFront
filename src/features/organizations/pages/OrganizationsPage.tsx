...
export const OrganizationsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        showNotification({
          type: "error",
          message: "Error al cargar datos de la organización",
        });
      } finally {
        setIsLoading(false);
      }
  ...
  const copyOrgId = () => {
    if (user?.organizationId) {
      ...
      showNotification({
        type: "success",
        message: "ID de organización copiado",
      });
    }
  };
    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto w-full flex flex-col gap-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          trend={
            activeProjects > 0
              ? { value: "En progreso", isPositive: true }
              : undefined
          }
        />
        <StatCard
          ...
          <div className="space-y-3">
            {Object.entries(roleDistribution)
              ...
              .map(([role, count]) => {
                const pct =
                  members.length > 0
                    ? Math.round((Number(count) / members.length) * 100)
                    : 0;
                return (
